...
export class RunningApiClient extends BaseApiClient {
  constructor() {
    super() // 기본 생성자로 프록시 활성화
  }

  async getTopRated(limit: number = 5, minReviews: number = 1): Promise<ApiResponse<CategoryTopRatedResponse>> {
    return this.fetchApi<CategoryTopRatedResponse>(`/running/top-rated?limit=${limit}&min_reviews=${minReviews}`)
  }

  async getTopByReviewCount(limit: number = 5, minReviews: number = 1): Promise<ApiResponse<TopByReviewCountResponse>> {
    return this.fetchApi<TopByReviewCountResponse>(`/running/top-by-reviewcount?limit=${limit}&min_reviews=${minReviews}`)
  }
  ...
  async getTopBySubcategory(subcategory: string, limit: number = 5, minReviews: number = 1): Promise<ApiResponse<SubcategoryTopResponse>> {
    return this.fetchApi<SubcategoryTopResponse>(`/running/top-by-subcategory?subcategory=${encodeURIComponent(subcategory)}&limit=${limit}&min_reviews=${minReviews}`)
  }

  async getPriceDistribution(binSize: number = 20000): Promise<PriceDistributionResponse> {
    // 프록시를 통해 호출
    const apiUrl = this.useProxy 
      ? `/api/proxy/running/price-distribution?bin_size=${binSize}`
      : `${this.baseUrl}/running/price-distribution?bin_size=${binSize}`
  ...
  async getWordCloud(topN: number = 100): Promise<WordCloudResponse> {
    // 프록시를 통해 호출
    const apiUrl = this.useProxy 
      ? `/api/proxy/running/wordcloud?top_n=${topN}`
      : `${this.baseUrl}/running/wordcloud?top_n=${topN}`
...
export const runningApi = new RunningApiClient()
