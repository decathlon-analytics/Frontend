        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
          <div className="p-3 bg-muted rounded-lg">
            <div className="text-lg font-bold text-foreground">
              {data.reduce((sum, bin) => sum + bin.n_products, 0)}
            </div>
            <div className="text-xs text-muted-foreground">총 상품수</div>
          </div>
          <div className="p-3 bg-muted rounded-lg">
            <div className="text-lg font-bold text-foreground">
              {(data.reduce((sum, bin) => sum + bin.avg_rating, 0) / data.length).toFixed(1)}★
            </div>
            <div className="text-xs text-muted-foreground">전체 평균 평점</div>
          </div>
          <div className="p-3 bg-muted rounded-lg">
            <div className="text-lg font-bold text-foreground">
              {Math.round(data.reduce((sum, bin) => sum + bin.avg_total_reviews, 0) / data.length)}
            </div>
